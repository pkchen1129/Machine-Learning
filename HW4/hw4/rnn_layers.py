...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    - Wx: Weight matrix for input-to-hidden connections, of shape (D, H) , Win
    - Wh: Weight matrix for hidden-to-hidden connections, of shape (H, H) , U
    - b: Biases of shape (H,)
    Returns a tuple of:
...
def temporal_fc_forward(x, w, b):
    ...
    N, T, D = x.shape
    M = b.shape[0]
    out_ = x.reshape(N * T, D) @ w #Have to reshape
    out = out_.reshape(N, T, M) + b #Have to reshape it to 
    cache = x, w, b, out
    return out, cache
